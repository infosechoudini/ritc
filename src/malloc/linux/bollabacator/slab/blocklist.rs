use crate::malloc::bollabacator::slab::blockheader::BlockHeader;
...
impl Block {
    /// Construct a `Block` from raw parts: a freed memory block at `ptr` of
    ...
    #[must_use]
    pub unsafe fn from_raw(ptr: NonNull<u8>, next: Option<Block>, size: usize) -> Block {
        debug_assert!(
            size >= HEADER_SIZE,
            "Can't recapture a block smaller than HEADER_SIZE"
        );
        let header = BlockHeader::from_raw(ptr, next, size);
        Block { header }
    }
    ...
    /// block in the list.
    #[must_use]
    pub fn decompose(mut self) -> (Range<NonNull<u8>>, Option<Block>) {
    ...
    #[must_use]
    fn replace_next(&mut self, new_next: Block) -> Option<Self> {
    ...
    pub fn header_view(&self) -> &BlockHeader {
    ...
    pub unsafe fn header_mut(&mut self) -> &mut BlockHeader {
        self.header.as_mut()
    }
    ...
    pub fn insert_merge(&mut self, block: Block) -> usize {
    ...
    /// Split off part of this Block, and return a pointer to the split off
    /// data.
    ///
    ...
    pub fn split(&mut self, size: usize) -> Range<NonNull<u8>> {
        ...
        unsafe {
            ...
            header.size -= size;
            let start =
                NonNull::new_unchecked((header as *mut BlockHeader as *mut u8).add(header.size));
            let end = NonNull::new_unchecked((header as *mut BlockHeader as *mut u8).add(self_size));
            // log::trace!(
            //     "Splitting {} bytes off from {:?}:{} to get {:?}",
            //     size,
            //     (header as *mut BlockHeader as *mut u8),
            //     self_size,
            //     ptr,
            // );
...
pub struct BlockIter<'list> {
    next: Option<&'list Block>,
}

impl<'list> Iterator for BlockIter<'list> {
    type Item = &'list Block;
...
impl fmt::Display for BlockList {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for block in self {
            ...
            write!(f, "Block({:?}, {})", block.header, block.size())?;
        }
...
impl BlockList {
    ...
    /// iteration.
    pub fn apply<C, R, F: FnMut(&mut Block, C) -> ApplyState<C, R>>(
        &mut self,
        start: C,
        mut pred: F,
    ) -> ApplyState<C, R> {
    ...
    pub unsafe fn add_block(&mut self, ptr: NonNull<u8>, size: usize) {
        let mut new_block = Block::from_raw(ptr, None, size);

        let first: &Block = match self.first {
            None => {
